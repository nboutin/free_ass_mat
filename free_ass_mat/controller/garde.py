...
class Garde:
    ...
    def get_heures_complementaires_mois(self, date: datetime.date) -> float:
        ...
        for week_number in week_numbers:
            h_comp_mois += self.get_heures_complementaires_semaine(date.year, week_number)
        return h_comp_mois
    ...
    def get_heures_majorees_mois(self, date: datetime.date) -> float:
        """Calculate the number of additional hours for a given month"""
        h_maj_mois: float = 0.0
        week_numbers = helper.get_week_numbers(date.year, date.month)

        for week_number in week_numbers:
            h_maj_mois += self.get_heures_majorees_semaine(date.year, week_number)
        return h_maj_mois
